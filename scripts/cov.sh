...
docker exec -i cov bash -c "cd /kratos/build/temp.linux-x86_64-3.8 && make -j2"
docker exec -i cov bash -c "cd /kratos/build/temp.linux-x86_64-3.8 && make test"
...
# clean up coverage
docker exec -i cov bash -c "cd kratos && lcov --remove coverage.info '/usr/*' '*extern*' '*tests*' '*python*' --output-file coverage.info"
